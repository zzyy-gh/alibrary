...
### Trigger 1: Tester Optimization Reports

The tester agent's optimize mode produces periodic reports surfacing entries needing attention. The human scans the report, confirms or corrects the flagged entries, and moves on. Corrections are semantic: "this is wrong," "this is missing a crucial caveat," "these two entries are actually about different things." The librarian translates the correction into properly structured, inference-optimised updates to the source nugget.
...
To empirically discover what "optimised for inference" means for the library's specific agents and domains, the librarian can produce variant versions of a nugget — one denser, one more verbose, one restructured differently — and measure which produces better retriever answers. The tester tracks outcomes over time and learns which refinement patterns tend to improve performance. This builds an evidence base for content design decisions rather than relying on intuition.
