...
When new material arrives, the indexer catalogues the raw item (frontmatter, tags, embedding) and synthesizes one or more initial nuggets from it. A single raw item may yield multiple nuggets if it covers distinct topics. Each nugget links back to its source via `derived-from`. After synthesizing, the indexer performs cross-reference discovery: scan existing nuggets by title and tags to find related items. For each match where the new nugget genuinely draws on existing content, create an additional `derived-from` edge with a `note` explaining the connection. All IDs must be resolved from file frontmatter using `resolve_id()` — never guessed or constructed from memory.
...
- **Relationship discovery:** After enriching an entry, scan for related entries. Create `derived-from` edges for dependencies and connections, and `contradicts` edges for incompatible claims. Lateral similarity is handled by vector search, not explicit edges. Also verify and strengthen provenance edges back to source raw items. All relationship IDs must be validated against existing library items before writing — use `resolve_id()` from helpers to look up exact UUIDs. Never construct or guess UUIDs from memory.
