...
- **Knowledge exceeds any interface we build for it.** Graphs give us nodes and edges, but a connection between two ideas is itself knowledge — it should be a node, which needs its own connections, and so on. The node/edge distinction is a projection we impose, not a property of knowledge itself. Every representation technique — graphs, embeddings, tags, hierarchies — is a scoping act: it illuminates certain connections while burying others. Embeddings make semantic similarity searchable but obscure structural and causal relationships. Graphs make explicit links traversable but miss latent connections. No single interface is neutral. This means the library must not over-commit to any one representation as "the native form." Instead, we layer complementary projections (embeddings for latent similarity, tags for human-curated axes, dense prose for context that resists quantization, provenance links for causal chains) and accept that novel, deep connections will often be found *between* these projections — in the gaps where one representation's blind spot is another's strength. Fast retrieval and deep discovery are in tension; the library's design must serve both rather than optimizing only for search speed.
...
- **Flat nuggets, rich connections.** `/nuggets/` is a flat folder — no subfolders, no hierarchy. Organization comes entirely from tags and vector embeddings — representations that encode relatedness intrinsically rather than through explicit link maintenance. Discovery is through search, not browsing a tree.
